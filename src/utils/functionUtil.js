...
// 下载文件 - React Native 版本
export async function exportFile(res, fileName, fileType) {
  try {
    const { Share, Platform, PermissionsAndroid } = require("react-native");
    const RNFS = require("react-native-fs");

    // 处理文件名
    let finalFileName = fileName;
    if (!finalFileName && res.headers && res.headers["content-disposition"]) {
      let index = res.headers["content-disposition"].indexOf("=") + 1;
      finalFileName = decodeURI(res.headers["content-disposition"].substring(index));
    }
    if (!finalFileName) {
    ...
    if (Platform.OS === "ios") {
      ...
      if (granted === PermissionsAndroid.RESULTS.GRANTED) {
        ...
        Alert.alert("下载成功", `文件已保存到: ${filePath}`);
      } else {
        const { Alert } = require("react-native");
        Alert.alert("权限不足", "无法保存文件，请授予存储权限");
      }
    }
  } catch (error) {
    console.error("文件下载失败:", error);
    const { Alert } = require("react-native");
    Alert.alert("下载失败", "文件下载过程中出现错误");
  }
}
/**
 * 显示层级数据
